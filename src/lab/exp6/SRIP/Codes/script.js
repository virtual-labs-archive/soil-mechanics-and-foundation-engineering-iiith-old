...
function experimentSetup(x) {
  ...
  if (x.matches) { // If media query matches
	...
	var upperFilled = draw.polygon ([[590,320],[590,280],[665,280],[665,70],[695,70],[695,280],[770,280],[770,320]]).fill('none').stroke({
		width: 3
	})
	...
	var lowerPartition1 = draw.line([[590,500],[590,540]]).stroke({
	  ...
	  var arrow1 = draw.line ([[695,230],[730,230]]).stroke({width: 1})
	  	
	  var text = draw.text('Stand Pipe')
text.move(740,220).font({ fill: '#f06', family: 'Inconsolata' })
	  
	  	  var arrow2 = draw.line ([[665,122],[540,122]]).stroke({width: 1})
	  	
	  
	  	  var text = draw.text('Time t1')
text.move(580,100).font({ fill: '#f06', family: 'Inconsolata' })
	  
	  var arrow3 = draw.line([[665,220],[570,220]]).stroke({width: 1})
	  
	  var text = draw.text('Time t2')
text.move(600,200).font({ fill: '#f06', family: 'Inconsolata' })
	  
	  	  var arrow4 = draw.line([[540,122],[540,450]]).stroke({width: 1})
		  
		  	  var text = draw.text('h1')
text.move(510,300).font({ fill: '#f06', family: 'Inconsolata' })
	  
	    var arrow5 = draw.line([[570,220],[570,450]]).stroke({width: 1})
		  
		  	  var text = draw.text('h2')
text.move(550,300).font({ fill: '#f06', family: 'Inconsolata' })
	  
	  	    var arrow6 = draw.line([[810,450],[810,150]]).stroke({width: 1})
			
			  var text = draw.text('h')
text.move(820,300).font({ fill: '#f06', family: 'Inconsolata' })
	  
	  var arrow7 = draw.line([[710,150],[810,150]]).stroke({width: 1})
	  
	    var arrow8 = draw.line([[710,140],[810,140]]).stroke({width: 1})
		
		var text = draw.text('dh')
text.move(815,135).font({ fill: '#f06', family: 'Inconsolata' })
